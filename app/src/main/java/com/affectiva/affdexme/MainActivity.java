...
public class MainActivity extends AppCompatActivity
    ...
    private boolean multiFaceModeEnabled = true; // 預設為false
    ...
    void initializeCameraDetector() {
         ...
         */
        detector = new CameraDetector(this, cameraType, cameraView, (multiFaceModeEnabled ? MAX_SUPPORTED_FACES : 3), Detector.FaceDetectorMode.SMALL_FACES);// 預設為1和LARGE_FACES
    ...
    public void onImageResults(List<Face> faces, Frame image, float timeStamp) {
        ...
        if (faces.size() <= 0) {
        ...
        } else if (faces.size() >= 1) {// 設定畫出的FACE數量???
            metricViewLayout.setVisibility(View.VISIBLE);

            Log.d("MainActivity/facesSize",faces.size()+"");
            ...
            for (MetricDisplay metricDisplay : metricDisplays) {
                //updateMetricScore(metricDisplay, faces.get(0)); // 預設只display第一個
                for(Face eachFace:faces){
                    updateMetricScore(metricDisplay,eachFace);
                }
            }
